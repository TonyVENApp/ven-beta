...
function formatEmailPrefixAsDisplayName(emailPrefix?: string | null) {
  const normalizedPrefix = normalizeName(emailPrefix);

  if (!normalizedPrefix) {
    return null;
  }

  const humanizedName = normalizedPrefix
    .split(/[._-]+/)
    .filter(Boolean)
    .map((part) => part.charAt(0).toUpperCase() + part.slice(1).toLowerCase())
    .join(' ');

  return humanizedName || normalizedPrefix;
}
...
function getSessionUserName(session: Session | null) {
  ...
  return (
    ...
    formatEmailPrefixAsDisplayName(session?.user.email?.split("@")[0])
  );
}
